...
<script src="{{ URL::asset('assets/libs/sweetalert2/sweetalert2.min.js') }}"></script>
    <script>
        ...
        var fetch_url = "{{ route('studentChatData') }}";
            ...
            $.ajax({
                ...
                success: function(data) {
                    $.each( data, function( key, value ) {
                            ...
                            if (value.receiver_avatar == null) {
                                img = '{{ URL::asset('assets/images/user.png') }}';
                            } else {
                                img = '{{ URL::asset('assets/images/user_avatars/') }}/'+value.receiver_id+'/'+value.receiver_avatar;
                            }
                            $('.chat-list').append(
                                '<a href="javascript:void(0);" onclick="fetch_data('+value.parent_id+', '+value.receiver_id+')" class="text-body chat-list-link">' + 
                                    '<div class="media p-2 chat_active_'+value.parent_id+'">' +
        ...
        function fetch_data(parent_id, receiver_id)
        {
            ...
            $.ajax({
                url: fetch_url,
                data: {'parent_id': parent_id, 'receiver_id' : receiver_id},
                success:function(data)
                {
                    $('.div_data').html(data);
        ...
        //Pusher.logToConsole = true;
        var channel = pusher.subscribe('teacher-reply');
        channel.bind('teacher-reply-chat', function(data) {
            populateChatList();
